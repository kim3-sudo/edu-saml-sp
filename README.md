...
- Anti-enumeration: all authentication failures return the same generic error message, regardless of the underlying reason (unknown user, disabled auto-provisioning, invalid assertion, etc.). Detailed reasons are only written to the PHP error log when `WP_DEBUG` is enabled.
...
5. **If not found and auto-provisioning is OFF:** login is denied with a generic error message. No information about whether an account exists is ever revealed to the browser — detailed reasons are only logged server-side when `WP_DEBUG` is `true`.
...
- All signature verification, response/assertion validation (expiry, audience restriction, destination checks) is delegated to the `onelogin/php-saml` library — this plugin does not implement any cryptographic or XML parsing logic itself.
- The plugin option (`edu_saml_sp_options`) is stored as a single, non-autoloaded WordPress option.
- Settings changes require the `manage_options` capability and are protected by WordPress's standard Settings API nonces.
- Break-glass account creation is nonce-protected and requires `manage_options`.
- Login failures always return an identical, generic error message to prevent user enumeration.
